...
BEGIN
  ...
  -- Check if profile exists by user_id (NOT by id, since trigger uses random UUID for id)
  SELECT id INTO v_profile_id FROM public.profiles WHERE user_id = v_user_id;
  
  IF v_profile_id IS NULL THEN
    INSERT INTO public.profiles (id, user_id, email, full_name)
    SELECT gen_random_uuid(), v_user_id, COALESCE(raw_user_meta_data->>'email', NEW.email), COALESCE(raw_user_meta_data->>'full_name', '')
  ...
  -- Insert business_users — use the profile id from profiles table
  SELECT id INTO v_profile_id FROM public.profiles WHERE user_id = v_user_id;
