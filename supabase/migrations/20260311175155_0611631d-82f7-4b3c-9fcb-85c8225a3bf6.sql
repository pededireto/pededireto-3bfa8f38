...
-- Fix create_affiliate_lead_with_business: correct column names
...
-- New RPC: link_affiliate_referral
-- Links a business registration/claim to an affiliate via ref code
...
DECLARE
  ...
  v_biz_name text;
BEGIN
  -- Resolve affiliate code to user
  SELECT ac.user_id INTO v_affiliate_id
  FROM affiliate_codes ac
  WHERE ac.code = p_ref_code AND ac.is_active = true
  LIMIT 1;
  ...
  -- Get business name if not provided
  IF p_business_name IS NULL THEN
    SELECT b.name INTO v_biz_name FROM businesses b WHERE b.id = p_business_id;
  ELSE
    v_biz_name := p_business_name;
  END IF;

  -- Check if lead already exists for this business+affiliate
  SELECT al.id INTO v_lead_id
  FROM affiliate_leads al
  WHERE al.affiliate_id = v_affiliate_id AND al.business_id = p_business_id
  LIMIT 1;

  IF v_lead_id IS NOT NULL THEN
    RETURN v_lead_id; -- already linked
  END IF;

  -- Create the lead
  INSERT INTO affiliate_leads (
    affiliate_id, business_id, business_name, source, status
  ) VALUES (
    v_affiliate_id, p_business_id, v_biz_name, 'referral_link', 'pending'
  )
  RETURNING id INTO v_lead_id;
