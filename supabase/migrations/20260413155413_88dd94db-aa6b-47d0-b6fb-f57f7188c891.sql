...
WITH plan_level AS (
  SELECT b.id,
    CASE
      ...
      -- Active subscription → look up tier from commercial_plans
      WHEN b.subscription_status = 'active'::subscription_status_tipo AND b.plan_id IS NOT NULL THEN
        COALESCE((SELECT cp.tier FROM commercial_plans cp WHERE cp.id = b.plan_id), 'free')
      -- Fallback: is_premium with active subscription
      WHEN b.subscription_status = 'active'::subscription_status_tipo AND b.is_premium = true THEN 'pro'
      ELSE 'free'
    END AS level
  FROM businesses b
)
