      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-6 max-w-[1400px]">
        <div className="space-y-3">
          ...
          <Button onClick={handleGenerate} disabled={!canGenerate} className="w-full h-12 font-display font-bold text-base" size="lg">
            {generating ? <><Loader2 className="h-5 w-5 mr-2 animate-spin" />A gerar...</> : "✦ Analisar Imagem + Gerar 5 Extensões"}
          </Button>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          {generating ? (
            ...
            <div className="h-full flex items-center justify-center p-8 text-center min-h-[400px]">
              <div><span className="text-5xl block mb-4">🎬</span><p className="text-sm text-muted-foreground leading-relaxed">Selecciona o objectivo, carrega uma imagem e clica em gerar.<br />Os dados do negócio e o tom são opcionais.</p></div>
            </div>
          )}
        </div>
      </div>
            ...
            {(result.extensoes || []).map((ext: any, i: number) => {
              ...
              return (
                <div key={ext.num} className={cn("rounded-2xl border bg-card overflow-hidden transition-all duration-300 hover:shadow-lg", c.border)}>
                  ...
                  <div className="px-4 py-3"><GrokBox content={ext.prompt} /></div>
                </div>
              );
            })}
          </div>
