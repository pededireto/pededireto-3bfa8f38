...
const TIERS: TierConfig[] = [
  { key: "free", label: "Gratuito", icon: Shield, description: "Perfil básico para começar", color: "border-border" },
  {
    key: "start",
    label: "START",
    icon: Star,
...
const PricingPage = () => {
  ...
  const getPlanForTier = (tierKey: string, cycle: BillingCycle) => {
    ...
    if (tierKey === "free") return plans.find((p) => p.price === 0 && p.plan_type === "business");
    if (tierKey === "start")
      return plans.find(
        (p) =>
    ...
    if (tierKey === "pioneiro") return plans.find((p) => p.name.toLowerCase().includes("pioneiro"));
    return null;
  };
  ...
  const getMonthlyEquivalent = (plan: any, tierKey: string) => {
    ...
    if (billing === "annual") return (plan.price / 11).toFixed(2);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {TIERS.map((tier) => {
                ...
                return (
                  <div
                    key={tier.key}
                    className={`relative rounded-2xl border-2 ${tier.color} bg-card p-6 flex flex-col ${tier.highlight ? "shadow-lg ring-2 ring-primary/20 scale-[1.02]" : "shadow-card"}`}
                  >
                    {tier.badge && (
                      <Badge
                        className={`absolute -top-3 left-1/2 -translate-x-1/2 text-xs px-3 ${tier.highlight ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"}`}
                      >
                        {tier.badge}
                      </Badge>
                    )}
                        ...
                        <>
                          <div className="text-3xl font-bold text-foreground">
                            ...
                            <span className="text-sm font-normal text-muted-foreground">
                              {tier.key === "pioneiro" ? "/ano" : "/mês"}
                            </span>
                          </div>
                          {billing === "annual" && plan && tier.key !== "pioneiro" && (
                            <p className="text-xs text-muted-foreground mt-1">Facturado €{plan.price.toFixed(2)}/ano</p>
                          )}
                        </>
                      ) : (
